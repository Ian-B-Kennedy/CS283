...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        if (fgets(cmd_buff, RDSH_COMM_BUFF_SZ, stdin) == NULL) {
            break;  // End-of-file (Ctrl+D) on stdin.
        }
        ...
        // Send the command (including the null terminator) to the server.
        if (send(cli_socket, cmd_buff, strlen(cmd_buff) + 1, 0) < 0) {
            perror("send");
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
        }

        // If the user types "exit" or "stop-server", break out of the loop.
        if (strcmp(cmd_buff, "exit") == 0 || strcmp(cmd_buff, "stop-server") == 0) {
            break;
        }

        // Receive loop: Keep receiving data until the EOF marker is seen.
        int finished = 0;
        while (!finished) {
            ...
            if (io_size < 0) {
                ...
                if (rsp_buff[io_size - 1] == RDSH_EOF_CHAR) {
                    // Print all received bytes except the EOF character.
