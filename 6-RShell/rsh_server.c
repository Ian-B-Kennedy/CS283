...
int exec_client_requests(int cli_socket) {
    static int last_rc = 0; // Tracks the last command's return code
    ...
    while (1) {
        ...
        if (io_size == 0) {
            ...
            // fprintf(stderr, "DEBUG: Client disconnected (recv returned 0).\n");
            break;
        }
        if (io_size < RDSH_COMM_BUFF_SZ)
            io_buff[io_size] = '\0';
        else
            io_buff[RDSH_COMM_BUFF_SZ - 1] = '\0';

        // Build a command buffer structure from the received command.
        command_list_t clist;
        int ret = build_cmd_list(io_buff, &clist);
        if (ret == WARN_NO_CMDS) {
            // No commands found, not a true error condition; just ignore.
        ...
        if (ret != OK) {
            send_message_string(cli_socket, "Error parsing command line");
            free(io_buff);
            continue;
        }

        // Determine if this is a built-in command.
        Built_In_Cmds built_in = rsh_built_in_cmd(&clist.commands[0]);

        switch (built_in) {
            case BI_CMD_EXIT:
                send(cli_socket, "exit", strlen("exit") + 1, 0);
                // send_message_string(cli_socket, "Exiting client connection");
                free(io_buff);
                return OK;  // Break out of the client loop; connection will be closed.
            case BI_CMD_STOP_SVR:
                send(cli_socket, "stop-server", strlen("stop-server") + 1, 0);
                free(io_buff);
                return OK_EXIT;  // Signal server shutdown.
            case BI_CMD_CD:
                // The cd command is handled in rsh_built_in_cmd (chdir is called there).
            ...
            case BI_CMD_RC: {
                ...
                send_message_string(cli_socket, buf);
                free(io_buff);
                break;
            }
            case BI_CMD_DRAGON: {
                // Instead of forking an external process, call the function directly.
                // Redirect stdout so that print_dragon() output is sent to the client.
            ...
            case BI_EXECUTED:
                // Built-in command was executed (e.g., cd) – nothing further to do.
                send_message_eof(cli_socket);
                free(io_buff);
                break;
            case BI_NOT_BI:
                // Execute the pipeline: the first command uses cli_socket as STDIN,
                // and the last command uses cli_socket as STDOUT and STDERR.
                ...
                break;
            default:
                // Not a built-in command; simply echo the command back.
                last_rc = send_message_string(cli_socket, io_buff);
                free(io_buff);
                break;
        }
    }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pid == 0) {  /* Child process */
            /* For the first command, use cli_sock as STDIN */
            if (i == 0) {
            ...
            /* For the last command, use cli_sock as STDOUT */
            if (i == clist->num - 1) {
...
Built_In_Cmds rsh_built_in_cmd(cmd_buff_t *cmd)
{
    ...
    case BI_CMD_CD:
        ...
        return BI_EXECUTED;
    case BI_CMD_DRAGON:
        // We choose to handle dragon as an external command.
        return BI_CMD_DRAGON;
    default:
        return BI_NOT_BI;
    }
